...
## Challenge
Implement `solveFunctionsBasics` in `06-Functions-Basics.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Functions-Basics` in the `topic` field.

## Pass Condition
Run: `npm run check 01-Easy-Fundamentals/06-Functions-Basics`
