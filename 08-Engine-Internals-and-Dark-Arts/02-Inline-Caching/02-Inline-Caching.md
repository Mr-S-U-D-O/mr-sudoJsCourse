...
## Challenge
Implement `solveInlineCaching` in `02-Inline-Caching.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Inline-Caching` in the `topic` field.

## Pass Condition
Run: `npm run check 08-Engine-Internals-and-Dark-Arts/02-Inline-Caching`

## Escape Hatch
https://v8.dev/docs/inline-caches
