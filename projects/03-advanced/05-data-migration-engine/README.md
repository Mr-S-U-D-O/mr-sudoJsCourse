...
Build a robust data migration engine implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/03-advanced/05-data-migration-engine/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/03-advanced/05-data-migration-engine/solution/index.solution'); console.log(m.metadata);"
```
