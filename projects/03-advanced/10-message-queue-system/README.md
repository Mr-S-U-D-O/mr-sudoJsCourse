...
Build a robust message queue system implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/03-advanced/10-message-queue-system/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/03-advanced/10-message-queue-system/solution/index.solution'); console.log(m.metadata);"
```
