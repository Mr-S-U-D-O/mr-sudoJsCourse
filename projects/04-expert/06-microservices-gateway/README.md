...
Build a robust microservices gateway implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/06-microservices-gateway/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/06-microservices-gateway/solution/index.solution'); console.log(m.metadata);"
```
