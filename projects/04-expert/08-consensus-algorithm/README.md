...
Build a robust consensus algorithm implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/08-consensus-algorithm/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/08-consensus-algorithm/solution/index.solution'); console.log(m.metadata);"
```
