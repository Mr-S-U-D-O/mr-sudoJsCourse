...
Build a robust performance profiler implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/10-performance-profiler/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/10-performance-profiler/solution/index.solution'); console.log(m.metadata);"
```
