...
Build a robust query optimizer implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/05-query-optimizer/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/05-query-optimizer/solution/index.solution'); console.log(m.metadata);"
```
