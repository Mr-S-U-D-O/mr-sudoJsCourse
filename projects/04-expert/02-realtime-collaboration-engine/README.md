...
Build a robust realtime collaboration engine implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/02-realtime-collaboration-engine/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/02-realtime-collaboration-engine/solution/index.solution'); console.log(m.metadata);"
```
