...
Build a robust plugin architecture implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/04-plugin-architecture/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/04-plugin-architecture/solution/index.solution'); console.log(m.metadata);"
```
