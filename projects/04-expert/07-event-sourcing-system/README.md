...
Build a robust event sourcing system implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/04-expert/07-event-sourcing-system/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/04-expert/07-event-sourcing-system/solution/index.solution'); console.log(m.metadata);"
```
