...
Build a robust calculator engine implementation with clear module boundaries and predictable behavior.
...
```bash
node -e "const m=require('./projects/01-beginner/01-calculator-engine/src'); console.log(Object.keys(m));"
```

2. Run the reference solution

```bash
node -e "const m=require('./projects/01-beginner/01-calculator-engine/solution/index.solution'); console.log(m.metadata);"
```
