...
## Challenge
Implement `solveBitwiseOperators` in `05-Bitwise-Operators.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Bitwise-Operators` in the `topic` field.

## Pass Condition
Run: `npm run check 04-Advanced-Deep-JS/05-Bitwise-Operators`
