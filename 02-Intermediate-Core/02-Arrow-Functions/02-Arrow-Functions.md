...
## Challenge
Implement `solveArrowFunctions` in `02-Arrow-Functions.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Arrow-Functions` in the `topic` field.

## Pass Condition
Run: `npm run check 02-Intermediate-Core/02-Arrow-Functions`
