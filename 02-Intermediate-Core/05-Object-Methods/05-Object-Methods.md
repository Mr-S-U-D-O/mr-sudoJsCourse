...
## Challenge
Implement `solveObjectMethods` in `05-Object-Methods.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Object-Methods` in the `topic` field.

## Pass Condition
Run: `npm run check 02-Intermediate-Core/05-Object-Methods`
