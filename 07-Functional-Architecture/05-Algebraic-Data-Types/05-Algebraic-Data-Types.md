...
## Challenge
Implement `solveAlgebraicDataTypes` in `05-Algebraic-Data-Types.js`.

Your function must:
- Accept an array of values as input.
- Return an object with `topic`, `total`, `truthyCount`, and `falsyCount`.
- Preserve the exact topic label `Algebraic-Data-Types` in the `topic` field.

## Pass Condition
Run: `npm run check 07-Functional-Architecture/05-Algebraic-Data-Types`
